...
export function shortHash(text: string): string {
  const [first, second] = [...text].reduce<Pair>(absorb, [SEED_LOW, SEED_HIGH]);
...
function absorb([first, second]: Pair, char: string): Pair {
...
function scramble(value: number, other: number): number {
