...
export function planMeta(type, title = "Faster checkout") {
    return {
        schemaVersion: 1,
...
export function seededHeadings(type) {
    return seedBlocks(templateFor(type), {
        title: "Faster checkout",
        idFor: (slot) => `${HEADING_PREFIX}${slot}`,
    });
}
export function planWith(type, content) {
    return seededHeadings(type).flatMap((block) => block.type === "section-panel"
        ? sectionOrder([block, ...(content[String(block.props["slot"])] ?? [])])
        : [block]);
}
/** A feature plan with everything approval asks for. */
export function readyFeature() {
    return planWith("feature", {
        intent: [textBlock("paragraph", {}, "Checkout takes too long.")],
        kpis: [
            textBlock("kpi", { kpiId: "k1", metric: "p95", baseline: "450 ms", target: "200 ms" }, "Every 100 ms costs a point of conversion."),
        ],
...
export function writtenBlocks(blocks = []) {
...
export function textBlock(type, props, text) {
    return parseBlock({
        id: `${type}-${text}`,
        type,
        props,
