...
import { tableCells } from "../blocks/prose-blocks.js";
import { DEFAULT_CODE_LANGUAGE } from "../ops/prose-input.js";
import { renderInline } from "./inline-render.js";
import { escapeBlockStart, listMarker, quoteLine, } from "./markdown-syntax.js";
const FENCE_MIN = 3;
...
const TEXT_ONLY = ["paragraph", "heading", "quote"];
const RENDERERS = {
...
export function writeProse(blocks) {
...
function listLines(block, number) {
    const type = block.type;
    const checked = block.props["checked"] === true;
    const marker = listMarker({ type, checked }, number);
    const indent = " ".repeat(type === "numberedListItem" ? marker.length + 1 : 2);
    ...
    const indented = (line) => (line === "" ? line : indent + line);
    return [
...
function leadingBackticks(line) {
...
function tableLines(rows) {
    ...
    const [header, ...body] = rows.map((cells) => [...cells, ...emptyCells(width - cells.length)].map(cellMarkup));
    if (!header || width === 0) {
        return [];
    }
    return [header, header.map(() => TABLE_RULE), ...body].map((cells) => `| ${cells.join(" | ")} |`);
}
function emptyCells(count) {
    return Array.from({ length: count }, () => []);
}
function cellMarkup(content) {
    return oneLine(content).replaceAll("|", "\\|");
}
//# sourceMappingURL=write-prose.js.map