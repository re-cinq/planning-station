...
export function refineInputs(blocks, slot) {
...
export function settledCount(inputs) {
...
export function markUsed(blocks, uses) {
    return blocks.map((block) => usedBy(block, uses)
        ? { ...block, props: { ...block.props, used: true } }
        : block);
}
function usedBy(block, uses) {
    if (block.type === "question") {
        return uses.questions.includes(block.props.questionId);
    }
    return (block.type === "comment" && uses.comments.includes(block.props.commentId));
}
function answeredIn(said, question) {
...
function threadsIn(comments) {
    return comments
        .filter((comment) => !comment.props.replyTo && isUnused(comment))
        .map((first) => ({
        first,
        replies: comments.filter((reply) => reply.props.replyTo === first.props.commentId),
        resolved: first.props.resolved,
    }));
}
function asThread({ first, replies }) {
    return {
...
//# sourceMappingURL=refine-inputs.js.map