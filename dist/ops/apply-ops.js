...
import { blocksOfType, isPlanBlock, parseBlock, } from "../blocks/block-json.js";
...
const addFinding = (blocks, op) => upsert(blocks, op.slot, findingBlock({ ...op, resolved: isResolved(blocks, op.findingId) }));
const HANDLERS = {
...
function isResolved(blocks, findingId) {
    const existing = blocksOfType(blocks, "finding").find((finding) => finding.props.findingId === findingId);
