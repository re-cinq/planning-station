...
export const disallowedBlocks = ({ plan: { sections }, template }) => sectionsWithSlots(sections, template.slots).flatMap(({ section: { blocks }, slot }) => blocks
...
export const emptyRequiredSections = ({ plan: { sections }, template: { slots }, phase, }) => isAtLeast(phase, "approval")
    ...
    : [];
export const blockRequirements = ({ plan: { sections }, template: { slots }, phase, }) => isAtLeast(phase, "approval")
    ? requiredSections(sections, slots, phase).flatMap(({ section, slot }) => slot.requires.flatMap((requirement) => countProblems(section.blocks, slot, requirement)))
    : [];
...
function sectionsWithSlots(sections, slots) {
    return sections.flatMap((section) => {
        const slot = slots.find((candidate) => candidate.slot === section.slot);
        return slot ? [{ section, slot }] : [];
    });
}
function requiredSections(sections, slots, phase) {
    return sectionsWithSlots(sections, slots).filter(({ slot }) => isRequiredAt(slot.required, phase));
}
function rankOf(rank, section) {
    return rank.get(section?.slot ?? "") ?? -1;
}
function allowedIn(slot, block) {
    return (slot.allows.includes(block.type) ||
        ALWAYS_ALLOWED.includes(block.type));
}
